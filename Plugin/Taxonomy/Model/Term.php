 ...
 */
	...
	//orly added
	public function getTermIdByValue($termValue, $alias) {
		$vocabulary = $this->Vocabulary->findByAlias($alias);
		if (!isset($vocabulary['Vocabulary']['id'])) {
			CakeLog::write('debug', 'failed');
			return false;
		}
		else{
			$vocabularyId = $vocabulary['Vocabulary']['id'];
			CakeLog::write('debug', $vocabularyId);
