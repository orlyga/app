...
class FacebookHelper extends AppHelper {
	...
	*/
	...
	function getLoginUrl($options = array(), $label = ''){
		$options = array_merge(
			array(
				'fields' => 'name,birthday,gender,location,email,first_name,last_name', 
				'redirect-uri' => Router::url($this->here, true),
				'width' => 350
			), 
			$options
		);
		return $this->__fbTag('fb:getLoginUrl',$label,$options);
	}
	/**
	* Register Button
	...
	public function init($options = null, $reload = true) {
		...
		if ($appId = FacebookInfo::getConfig('appId')) {
			...
			if($this->action<>'login') 
				$check_logged="FB.getLoginStatus(function(response) {
			if (response.authResponse) {
				// logged in and connected user, someone you know
				 if (typeof ajax_login ==='function') { ajax_login('facebook_id',response.authResponse.userID);}
			} else {
