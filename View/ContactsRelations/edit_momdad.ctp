...
?>
 ...
 if($(data).find("#request_status_id").val()=="success"){
	...
	else
	  {
	  	...
	  	if (parent1==8)  	$("#replacefirst").html($(data).find("#sec_parent_fields").html());
	  	return false;
		...
		<div id="firstparent" class="grid_4">
			<div class='nicebox'>
				...
				'birthdate'=>array('type'=>'hidden'),
					...
					echo $this->element('contacts/form', array('index'=>$i,'fieldset'=>$contact_fields,'hostingdiv'=>'replacefirst'));
			 		  ?>
				</div>
			</div>
			<?php $this->Form->unlockField('Family.familyrole_id');
				echo  $this->Js->submit(__('Submit'),
					  array( 'url'=> array('controller'=>'families',
					   'action'=>'edit',$this->data['Family']['id'],"edit_momdad"),
					   'before'=>'var $this=$(this);',
					    'buffer' => false,
					    'class'=>"button-elegant absolute_center",
					    'success'=>'updateFamilyContact(data,"first-parent");',
					    ));?>
		</div>
